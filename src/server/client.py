...
while conta<2:
    ...
    if conta==0:
        ip_server=data.decode('utf-8').split(";")[0]
        porta_server=data.decode('utf-8').split(";")[1]
    conta+=1
    server_socket.close()

print(f"{ip_server};{porta_server}")
if(response_str.split(";")[1]=="2"):
    time.sleep(2)
# Crea un socket TCP/IP
server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
# Associa il socket all'indirizzo e alla porta del server
server_socket.bind((client_ip, client_port))
server_socket.connect((ip_server, int(porta_server)))
...
# Ricevi la risposta dal server
response = server_socket.recv(1024)  # Ricevi fino a 1024 byte di dati
